...
1. 讀取 Vault `CLAUDE.md`，提取 `vault_path`、`vault_name`、`plugin_version`、`interaction_mode`、`cli_write_mode`
   - 缺 `interaction_mode` → 預設 `human`（v0.8 vault 相容）
...
本 skill 與其 sub-agent / commands 中的 `obsidian create / append / eval` 範例**僅適用於 `cli_write_mode: cli_first`**。
若 `cli_write_mode: native_only`，所有寫入命令一律改用 Claude Code 原生 Read/Edit/Write，
對照表見 `${CLAUDE_PLUGIN_ROOT}/references/cli-usage.md` 的「Mode 對照表」段落。
主對話必須將 `cli_write_mode` 傳入每個 sub-agent prompt，sub-agent 收到後依此決定寫入路徑。
